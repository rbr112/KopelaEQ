...
class ContextMock{constructor(){lastContext=this;this.state='running';this.currentTime=0;this.sampleRate=48000;this.destination=new NodeMock('dest');this.listeners=new Map()}createMediaStreamSource(){return new NodeMock('src')}createGain(){return new GainMock()}createBiquadFilter(){return new BiquadMock()}createDynamicsCompressor(){return new CompressorMock()}createAnalyser(){return new AnalyserMock()}createChannelSplitter(){return new NodeMock('split')}addEventListener(type,fn){const a=this.listeners.get(type)||[];a.push(fn);this.listeners.set(type,a)}emit(type){for(const fn of this.listeners.get(type)||[])fn()}async resume(){this.state='running';this.emit('statechange')}async suspend(){this.state='suspended';this.emit('statechange')}}
globalThis.AudioContext=ContextMock;
let listener=null,gumCalls=0,defer=false,resolver=null,stopCount=0,lastStream=null,lastContext=null;
