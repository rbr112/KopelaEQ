...
let listener=null, removed=null, statusChanged=null;
globalThis.chrome={
...
assert.equal(typeof listener,'function'); assert.equal(typeof removed,'function'); assert.equal(typeof statusChanged,'function');
...
let r=await invoke({type:S.MessageType.StatusGet,tabId:1}); assert.equal(r.ok,true); assert.equal(r.state.gainDb,4); assert.equal(r.state.eq.gains[0],3.25); assert.equal(r.state.dynamics.enabled,false); assert.equal(r.state.stereo.enabled,false); assert.equal(store[S.STORAGE.AUDIO_BASELINE_VERSION],3);
